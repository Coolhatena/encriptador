let matrizCodificadora = [
  [1, -1, -1, 1],
  [2, -3, -5, 4],
  [-2, -1, -2, 2],
  [3, -3, -1, 2],
];
let matrizDecodificadora = [
  [6, -1, 0, -1],
  [22, -4, 1, -4],
  [14, -3, 1, -2],
  [31, -6, 2, -5],
...
function numeroAChar(texto) {
  ...
  for (let i = 0; i < texto.length; i++) {
    resultado += String.fromCharCode(
      texto.charCodeAt(i) + (texto.charCodeAt(i) == 32 ? 33 : 21)
    );
  }
...
function charANumero(texto) {
  ...
  for (let i = 0; i < texto.length; i++) {
    resultado += String.fromCharCode(
      texto.charCodeAt(i) - (texto.charCodeAt(i) == 65 ? 33 : 21)
    );
  }
...
function desencriptarTexto() {
  ...
  document.getElementById("textoDecodificado").value =
    textoDesdeMatriz(matrizPlana);
} //Fin de funcion
